        ...
        for line in lines:
            if( len(line) > 0 ):
                ...
                if decodedData[0] == "REG":
                    ...
                    # Then read sender_pid, and check if its priority is 1.
                    # If priority is 1, then start things and set start_flag to false
                    
                    sender_pid = decodedData[1].split(".")[0]
                    if  clientMap[sender_pid]["priority"] == "1":
                        startMessaging = True

                    if startMessaging == True:
                        if( Iterator == 0 ):
                            ...
                            sender_pid = decodedData[1].split(".")[0]
                            queue.append( {
                                "priority": clientMap[sender_pid]["priority"],
                                "data": data
                            })
